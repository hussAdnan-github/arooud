...
<div className="mt-6">
<label for="first_name" className="block mb-4 text-sm font-medium text-gray-500 dark:text-white">صورة </label>
...
<input className="  text-endblock w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" id="file_input" type="file" />
        ...
        {/* <TableBody className="bg-blue-700 rounded-full"> */}
          {table.getRowModel().rows.length ? (
            table.getRowModel().rows.map((row) => (
              <div className="w-full flex justify-between bg-white rounded-lg items-center p-2"
                key={row.id}
    ...
    {/* <div className="flex items-center justify-end space-x-2 py-4">
      ...
      <div className="flex items-start gap-4 absolute bottom-10 p-3 rounded-xl bg-white">
         ...
         <div>
         <button className="me-4 text-primaryColo" onClick={()=>{table.previousPage()}} disabled={!table.getCanPreviousPage()}><FaArrowLeft/></button>
          <button className="text-primaryColo " onClick={()=>{table.nextPage()}} disabled={!table.getCanNextPage()}><FaArrowRight/></button>
         </div>
            <div className="flex items-center gap-1">
            <h1 className="text-black text-sm font-bold">
              {table.getState().pagination.pageIndex+1}  {' - '} {table.getPageCount()} {'of'}{' 28'}
            </h1>
            </div>
      </div>
    </div>
